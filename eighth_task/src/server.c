    ...
    for (int i = 0; i < FD_SETSIZE; ++i) { //бежим по всем соккетам
      if (FD_ISSET(i, &readset)) { // Определяем тип события и выполняем соответствующие действия
        if (i == sockfd) { // Поступил новый запрос на соединение, используем accept
          ...
          if (!strcmp("time", buff)) {
            ifTime = !ifTime;
            sendBuff = ctime(&curTime);
          }
          else
             sendBuff = readFile(buff); // вызываем функцию чтения данных из файла

          // Отправляем данные обратно клиенту
          if (send(i, sendBuff, strlen(sendBuff)*sizeof(char), 0) < 0) {
            perror("send");
            exit(4);
          }
          if(!ifTime)
            free(sendBuff); //освобождаем память
        }
      }
    }
  }
